...
const CreateLayout = () => {
    ...
    const [newItem, setNewItem] = useState({ i: "", type: "text", content: "", title: "" });
    ...
    const fetchSummaryEm = async () => {
        try {
            ...
            setSummaryEmployee(data.content);
        } catch (error) {
            alert(error)
        }
    }
    ...
    const addItem = async () => {
        ...
        setNewItem({ i: "", type: "text", content: "", title: "" });
                ...
                {(() => {
                    switch (item.type) {
                        case "text":
                            return (
                                <div>
                                    {item.title != "" && (<div style={{
                                        textTransform: "uppercase",
                                        fontSize: "22px",
                                        fontWeight: "400",
                                        marginBottom: "15px",
                                        borderBottom: "solid 5px #cf2e2e",
                                        width: "fit-content",
                                        padding: "3px 2px"
                                    }}>{item.title}</div>)}
                                    <div
    ...
    const generateHTML = () => {
        ...
        const layoutHTML = layouts.lg.map((layoutItem) => {
            ...
            switch (item.type) {
                case "text":
                    contentHTML = `<div>
                                    <div style=" text-transform: uppercase; font-size: 25px; font-weight: 400; margin-bottom: 15px; border-bottom: solid 5px #cf2e2e; width: fit-content; padding: 3px 2px">${item.title}</div>
                                    <div>${item.content || "Default Text"}</div>
                                </div>`;
                    break;
                case "input":
        ...
        <div className={cx("main-container")}>
            <div style={{
                ...
                <Select style={{ width: "200px" }} onChange={handleChange}>
                    {summaryEmployee.map((item) => (
                        <Option value={item.idUser}>{item.tenNhanVien}</Option>
                    ))}
                </Select>

...
import {
  Box, TextField, MenuItem, Grid, Select, InputLabel, FormControl, Checkbox, ListItemText, OutlinedInput, FormHelperText, SelectChangeEvent
} from '@mui/material';
...
const AddClassForm: React.FC<AddClassFormProps> = ({ classData, onSubmit, id }) => {
  ...
  const handleDayOfWeeksChange = (e: SelectChangeEvent<number[]>): void => {
    const { value } = e.target;
    setForm(prev => ({
      ...prev,
      schedule: {
        ...prev.schedule,
        dayOfWeeks: typeof value === 'string' ? value.split(',').map(Number) : value as number[]
      }
    }));
  };

  const handleTimeSlotSelect = (timeSlotValue: string): void => {
  ...
  const handleBlur = (field: string): void => {
    ...
    const error = validateField(field, form[field as keyof FormData], !!classData);
    if (error) {
      setErrors(prev => ({
        ...prev,
        [field]: error
  ...
  const handleScheduleBlur = (field: string): void => {
    // Validate field on blur
    const error = validateField(`schedule.${field}`, form.schedule[field as keyof Schedule], !!classData);
    if (error) {
      setErrors(prev => ({
        ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    try {
      ...
      const sortedDays = [...form.schedule.dayOfWeeks].map(day => parseInt(day.toString())).sort((a, b) => {
